...
void Display(void)
{
	...
	for (long faceIDs = 0; faceIDs < nFaces; faceIDs++) {
		calFaceNormal(Data.faces[faceIDs].vertIDs[0], Data.faces[faceIDs].vertIDs[1], Data.faces[faceIDs].vertIDs[2], normal);
		glNormal3f(normal[0], normal[1], normal[2]);
		for (int p = 0; p < 3; p++)
			glVertex3f(Data.verts[Data.faces[faceIDs].vertIDs[p]].x,
				Data.verts[Data.faces[faceIDs].vertIDs[p]].y,
				Data.verts[Data.faces[faceIDs].vertIDs[p]].z);
	}
	glEnd();
